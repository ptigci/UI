"""The Pasifik has no wifi, and the operator pressed GIT PULL.
...
from competitions.international_uav.config import DEVELOPER_WIFI, WIFI_DIALOG_TEXT
...
class WifiDialog(QDialog):
    ...
    def __init__(self, vehicle: Vehicle, state: WifiState, action_text: str, parent=None) -> None:
        ...
        title_label = QLabel(self.title_text(state), self)
        title_label.setWordWrap(True)
        set_role(title_label, ROLE_PAGE_TITLE)
        card.add_widget(title_label)
        state_label = QLabel(self.state_text(state), self)
        state_label.setWordWrap(True)
        set_role(state_label, ROLE_HINT)
        card.add_widget(state_label)
        hint_label = QLabel(WIFI_DIALOG_TEXT["hint"].format(action=action_text), self)
        ...
        card.add_widget(self.fixed_row(self.build_form_row()))
        self.status_label = QLabel(self)
        self.status_label.setWordWrap(True)
        set_role(self.status_label, ROLE_HINT)
        card.add_widget(self.status_label)
        card.add_widget(self.fixed_row(self.build_button_row()))
    ...
    def state_text(state: WifiState) -> str:
        return WIFI_DIALOG_TEXT["state_line"].format(
            ethernet_state=state.ethernet_state(), ethernet_address=state.ethernet_address(),
            wlan_device=DEVELOPER_WIFI["wlan_device"], wlan_state=state.wlan_state(),
        )
    ...
    def rescan(self) -> None:
        ...
        self.set_busy(True)
        self.run(wifi_scan(self.vehicle, self), self.scan_finished)
    ...
    def scan_finished(self, exit_code: int, lines: list[str]) -> None:
        ...
        self.networks = parse_wifi_scan(lines)
        self.table.setRowCount(len(self.networks))
        for row_index, network in enumerate(self.networks):
            if network.secured:
            ...
            self.table.setItem(row_index, SIGNAL_COLUMN, QTableWidgetItem(str(network.signal)))
    ...
    def connect_network(self) -> None:
        ...
        self.set_busy(True)
        self.run(
            wifi_connect(self.vehicle, network.ssid, self.password_input.text(), self),
    ...
    def check_connectivity(self) -> None:
        ...
        self.run(connectivity_check(self.vehicle, self), self.connectivity_answered)

    def connectivity_answered(self, exit_code: int, lines: list[str]) -> None:
        if last_line(lines) != DEVELOPER_WIFI["connectivity_full"]:
            return
        self.wait_timer.stop()
