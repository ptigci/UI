...
The computer node runs here, and so do the drone nodes in simulation: SITL and
the UDP mesh live on this machine, so there is no ssh in the way. The push to a
Pi runs here as well, because it is this laptop's checkout that sends it. Each
is a QProcess with the same line signals as an ssh session, so the tab shows
...
from competitions.swarm_uav.config import DEVELOPER_LOCAL, DEVELOPER_STOP_WAIT_MS

logger = logging.getLogger(__name__)

LINE_SEPARATOR = "\n"
START_FAILURE_EXIT_CODE = -1
# competitions/swarm_uav/dev/local_process.py -> the UI root.
...
class LocalProcess(QObject):
    ...
    def stop(self) -> None:
        """Kill it. Windows cannot ask a console process to close, so a node
        does not get the clean shutdown Ctrl+C would give it."""
