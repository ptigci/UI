"""Reading one line of the vtol unit's journal.

The node logs through colorlog, so a line arrives wrapped in the escape codes
...
from competitions.suas.config import DEVELOPER_LOG_STATES
...
def decode_log_line(line: str) -> tuple[str, str | None]:
    ...
    Anything the node does not use — the reset at the end of every line, the
