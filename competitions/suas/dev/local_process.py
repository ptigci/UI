...
The ground services run here, and so does the vtol node in simulation: SITL
lives on this machine, so there is no ssh in the way. The push to the Pi runs
here as well, because it is this laptop's checkout that sends it. Each is a
QProcess with the same line signals as an ssh session, so the tab shows its
output the same way.

Each [developer.local.*] table says what to run in its own folder: the two
applications run their main.py, and the test mission runs the operator tool
with its arguments.
"""
...
from competitions.suas.config import DEVELOPER_LOCAL, DEVELOPER_STOP_WAIT_MS

logger = logging.getLogger(__name__)

LINE_SEPARATOR = "\n"
START_FAILURE_EXIT_CODE = -1
# competitions/suas/dev/local_process.py -> the UI root.
...
def local_app(app: dict, parent=None) -> LocalProcess:
    """One of the apps under [developer.local]: its own python and its script."""
    return LocalProcess(
        local_python(app),
        [app["script"], *app["arguments"]],
        app["directory"],
